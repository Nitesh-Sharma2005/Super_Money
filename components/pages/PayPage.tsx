...
// Helper to parse UPI QR code data
const parseUpiUrl = (url: string): PayeeInfo | null => {
  try {
    ...
    if (payeeId && payeeName) {
      return { name: payeeName, upiId: payeeId };
    }
    return null;
  } catch (e) {
...
const PayPage: React.FC<PayPageProps> = ({ transactions, onAddTransaction, onDeleteTransaction, onDeleteMultipleTransactions, onClearAllTransactions }) => {
  ...
  if (view === 'scanning') {
    return <QRScanner onCancel={handleBack} onScanSuccess={handleScanSuccess} />;
  }
...
const QRScanner: React.FC<{ onCancel: () => void; onScanSuccess: (payee: PayeeInfo) => void; }> = ({ onCancel, onScanSuccess }) => {
                    ...
                    if (videoRef.current && !videoRef.current.paused && videoRef.current.readyState === 4) {
                        try {
                            ...
                            if (barcodes.length > 0) {
                                ...
                                cleanup();
                                const upiData = parseUpiUrl(barcodes[0].rawValue);
                                if (upiData) {
    ...
    return (
      <div className="relative flex flex-col items-center justify-center h-full bg-black text-white p-4 overflow-hidden">
        ...
        <div className="relative w-64 h-64 z-10">
          {/* Corner brackets */}
          <div className="absolute -top-1 -left-1 w-12 h-12 border-t-4 border-l-4 border-purple-500 rounded-tl-lg"></div>
          <div className="absolute -top-1 -right-1 w-12 h-12 border-t-4 border-r-4 border-purple-500 rounded-tr-lg"></div>
          <div className="absolute -bottom-1 -left-1 w-12 h-12 border-b-4 border-l-4 border-purple-500 rounded-bl-lg"></div>
          <div className="absolute -bottom-1 -right-1 w-12 h-12 border-b-4 border-r-4 border-purple-500 rounded-br-lg"></div>
          {/* Scanning laser */}
          <div className="absolute top-0 w-full h-1 bg-purple-400/80 shadow-[0_0_15px_2px_rgba(192,132,252,0.8)] animate-scan"></div>
        </div>
        
        <p className="mt-8 text-lg font-medium text-gray-300 z-10">Searching for QR code...</p>
        ...
        <style>{`
          ...
          .animate-scan {
            animation: scan 2s infinite cubic-bezier(0.5, 0, 0.5, 1);
          }
