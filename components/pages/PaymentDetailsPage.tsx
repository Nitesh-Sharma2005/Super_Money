...
const playProcessingSound = () => {
  const audio = new Audio( 'https://github.com/Nitesh-Sharma2005/Super_Money/raw/main/payment.mp3');
  audio.play().catch((e) => console.error('Audio playback failed:', e));
};
  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-800 font-sans">
      <header className="flex items-center justify-between p-4 border-b border-gray-200">
        ...
        <h1 className="text-lg font-bold">Paying</h1>
      ...
      <main className="flex-1 flex flex-col items-center p-6">
        <div className="flex flex-col items-center text-center">
          <div className="w-16 h-16 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-2xl font-bold mb-3">
            {currentPayee.name.charAt(0).toUpperCase()}
          </div>
          <p className="font-semibold text-lg">{currentPayee.name}</p>
          <p className="text-sm text-gray-500">{currentPayee.upiId}</p>
        </div>
        ...
        <div className="w-full my-auto text-center">
          <div className="relative inline-block">
            ...
            <input
              ...
              placeholder="0"
              className="bg-transparent text-6xl font-bold text-center w-full pl-8 focus:outline-none"
              autoFocus
            />
          </div>
        </div>
      ...
      <footer className="p-4 border-t border-gray-200">
        <div className="flex justify-between items-center bg-gray-100 p-3 rounded-lg mb-4">
            <div>
                <p className="text-sm text-gray-600">Paying from</p>
                <p className="font-semibold">Super Savings Account</p>
            </div>
            <div className="flex items-center space-x-2">
                <YesBankLogo className="h-4 text-gray-700" />
                <RuPayLogo className="h-4 text-gray-700" />
                <UpiLogo className="h-4 text-gray-700" />
            </div>
        </div>
        <button
          onClick={handleProceedClick}
          disabled={!amount || parseFloat(amount) <= 0}
          className="w-full py-4 text-lg font-semibold bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all"
        >
          Proceed to Pay
        </button>
      </footer>
